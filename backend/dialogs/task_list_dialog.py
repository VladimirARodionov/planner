...
async def get_tasks_data(dialog_manager: DialogManager, **kwargs):
    ...
    async with get_session() as session:
        ...
        for task in tasks:
            ...
            deadline = escape_html((task['deadline'].strftime("%d.%m.%Y %H:%M").replace(tzinfo=pytz.timezone(user.timezone)) if task['deadline'] else "") + ((" " + i18n.format_value("deadline-overdue")) if task['deadline'] and not task['completed_at'] and task['deadline'] < datetime.now(tz=pytz.timezone(user.timezone)) else "") if task['deadline'] else i18n.format_value("deadline-not-set"))
